...
export async function addUsertoDb(userInfo) {
  const { name, username=null, email, password_hash = null, google_id = null } = userInfo;

  const query = `
    INSERT INTO users (name,username, email, password_hash, google_id)
    VALUES ($1, $2, $3, $4, $5)
    ON CONFLICT (email) 
    DO UPDATE SET 
  ...
  try {
    ...
    const result = await client.query(query, [name,username, email, password_hash, google_id]);
    client.release();
