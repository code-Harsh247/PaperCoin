...
export async function addUsertoDb(userInfo) {
  ...
  try {
    ...
    if (checkResult.rowCount > 0) {
      // User exists, update but don't return id
      const updateQuery = `
        UPDATE users 
      ...
      await client.query(updateQuery, [password_hash, google_id, email]);
      client.release();
      
      console.log("User already exists, updated but not returning id");
      return null;
    } else {
      // User doesn't exist, insert and return id
      const insertQuery = `
      ...
      const insertResult = await client.query(insertQuery, [username, email, password_hash, google_id]);
      client.release();
      
      console.log("New user added with id:", insertResult.rows[0].user_id);
